...
## Skills disponibles (uso automático)

Al trabajar con archivos de esta bóveda, usar las skills correspondientes:

- `obsidian-markdown` → para archivos `.md` con wikilinks, callouts, frontmatter
- `obsidian-bases` → para archivos `.base` con vistas y filtros
- `json-canvas` → para archivos `.canvas`
- `dataview` → para queries con el plugin Dataview
- `datacore` → para componentes reactivos con Datacore
- `tasks` → para gestión de tareas con el plugin Tasks
- `templater` → para plantillas dinámicas con Templater
